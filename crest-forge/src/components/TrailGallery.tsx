...
export const TrailGallery: React.FC = () => {
  return (
    <section id="trails" className="bg-crest-charcoal py-16 md:py-24 px-6 md:px-12 lg:px-20">
        ...
        {trails.map((trail) => (
          <FadeInStaggerItem key={trail.id} className={`${trail.span} relative rounded-[1.5rem] overflow-hidden group cursor-pointer`}>
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-6 flex flex-col justify-end h-full">
              <h3 className="font-heading italic text-crest-white text-xl md:text-2xl drop-shadow-md">
                {trail.name}
              </h3>
              
              <div className="flex items-center mt-2 overflow-hidden h-8">
                <div className="liquid-glass rounded-full px-3 py-1 text-xs text-crest-white/80 shrink-0 transform transition-transform duration-400 ease-out group-hover:-translate-y-[150%]">
                  {trail.location}
                </div>
                <div className="absolute flex items-center text-crest-ember text-sm font-body font-medium transform translate-y-[150%] opacity-0 transition-all duration-400 ease-out group-hover:translate-y-0 group-hover:opacity-100">
