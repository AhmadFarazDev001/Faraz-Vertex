...
export const Testimonials: React.FC = () => {
  return (
    <section id="about" className="bg-crest-charcoal py-16 md:py-24 px-6 md:px-12 lg:px-20">
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="text-center max-w-3xl mx-auto">
          ...
          <BlurText 
            text={"What they say\nafter the drop."} 
            className="font-heading italic text-crest-white text-[clamp(2.5rem,5vw,4.5rem)] leading-[0.9] tracking-[-2px] justify-center" 
          />
        </div>
        ...
        <FadeInStaggerGroup className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12 md:mt-16">
          {testimonials.map((t) => (
            <FadeInStaggerItem key={t.id} className="h-full">
              <div className="liquid-glass rounded-[1.5rem] p-8 flex flex-col h-full border border-transparent hover:border-crest-white/10 transition-colors">
                
                {/* Stars */}
                <div className="flex gap-1">
                  {[...Array(5)].map((_, i) => (
                    <svg key={i} className="h-4 w-4 text-crest-ember fill-current" viewBox="0 0 20 20">
                ...
                <div className="mt-4 relative flex-grow">
                  <span className="absolute -left-2 -top-4 font-heading text-5xl text-crest-ember/30 leading-none select-none">
                    "
                  </span>
                  <p className="text-base text-crest-white/90 font-body font-light leading-relaxed italic relative z-10 pl-2">
                    {t.quote}
                  </p>
                </div>
                
                <div className="mt-8 pt-6 border-t border-crest-white/5 flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full overflow-hidden shrink-0">
                    <img src={t.avatar} alt={t.name} className="w-full h-full object-cover" />
                  </div>
                  <div className="flex flex-col">
                    <span className="text-sm font-body font-semibold text-crest-white">{t.name}</span>
                    <span className="text-xs text-crest-sand/60 font-body">{t.title}</span>
                  </div>
                </div>
